    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # for j in range(len(path[0])):
                #     dx = path[0][j] - goal_state[0]
                #     dy = path[1][j] - goal_state[1]
                #     dist = np.sqrt(dx**2 + dy**2)
                #     score += dist**2
                # score = np.sqrt(score) # score is MSE between centerline and path
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            # --------------------------------------------------
                            otherPath = paths[j]
                            proximityScore = np.sqrt((path[0][-1] - otherPath[0][-1])**2 + (path[1][-1] - otherPath[1][-1])**2)

                            # proximityScore = 0
                            # curPathLength = len(path[0])
                            # otherPath = paths[j]
                            # otherPathLength = len(otherPath[0])
                            # m = min(curPathLength, otherPathLength)
                            # for k in range(m):
                            #     dx = path[0][k] - otherPath[0][k]
                            #     dy = path[1][k] - otherPath[1][k]
                            #     dist = np.sqrt(dx**2 + dy**2)
                            #     proximityScore += dist**2
                            # proximityScore = np.sqrt(proximityScore)

                            score += self._weight * proximityScore
                            # --------------------------------------------------
